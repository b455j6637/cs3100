// 02/04/2017 - CS 3100 - Meine, Joel
// Threadpool MandelBrot
...
int task_types = 5; // number of task types
...
void m_image_p(int x, int y, int x_limit, int y_limit, int thread)
{
	...
	if (x == x_limit && y == y_limit) // is start and end point the same?
	...
	m_colors_p[thread] = s_m_colors; // save pixel colors to image
}

// mandelbrot image, parallel [1] (initialize image)
void m_image_pI(int tasks)
{
	for (int i = 0; i < tasks; i++)
		m_colors_p.push_back({});
}
...
// splice image, multiple pixels on one-pixel height row
void s_image_line(bool isFull)
{
	s_points_start = {}; s_points_end = {}; // clear spliced image points
	for (int i = 0; i < height; i++) // for every one-pixel height row
	{
		s_points_start.push_back({ 0, i }); // start at first pixel point of row
		if (isFull == false) // set pivot points if not a full row
		{
			int pivot = width / 2;
			s_points_end.push_back({ pivot, i }); // end at pivot pixel point of row
			s_points_start.push_back({ pivot + 1, i }); // start after pivot pixel point of row
		}
		s_points_end.push_back({ width - 1, i }); // end at last pixel point of row
	}
}

// splice image, blocks of pixels
void s_image_block(int cols, int rows, bool isEven)
{
	s_points_start = {}; s_points_end = {}; // clear spliced image points
	int s_unit_width = width / cols; // width of each spliced image column
	int s_unit_height = height / rows; // height of each spliced image row
	for (int i = 0; i < rows; i++) // every row of spliced image
	{
		for (int j = 0; j < cols; j++) // every column of spliced image
		{
			int x = j*s_unit_width; int y = i*s_unit_height; // save block point
			// save top-left splice point of block
			if (isEven == true)
				s_points_start.push_back({ x, y });
			else
			{

			}
			// save bottom-right splice point of block
			if (i == rows - 1 && y + (s_unit_height - 1) != height - 1) // last row?
				s_points_end.push_back({ x + (s_unit_width - 1), height - 1 }); 
			else
				s_points_end.push_back({ x + (s_unit_width- 1), y + (s_unit_height - 1) });
		}
	}
}
...
// run task
void task_run(int t_number, bool isOnePixel)
{
	int task_limit;
	int task_id = 0;
	// check to ensure task number is correct
	if (s_points_start.size() == s_points_end.size())
		task_limit = s_points_start.size();
	else
		cout << "ERROR!" << endl;

	ThreadPool t_pool(t_number + 1); // initialize threadpool
	
	if (isOnePixel == false) // not 1 pixel per task
	{
		if (m_colors_p.size() == 0)
			m_image_pI(task_limit); // intialize the mandelbrot point colors of image, parallel
		time_length_p = timer(true); // start timer
		for (int i = 0; i < task_limit; i++)
		{
			// add task to threadpool
			t_pool.post([=]() {m_image_p(s_points_start[i][0], s_points_start[i][1], s_points_end[i][0], s_points_end[i][1], task_id); });
			task_id++;
		}
		t_pool.start(); // start the threadpool
		time_length_p = timer(false); // end timer
	}
	else // 1 pixel per task
	{		
		if (m_colors_p.size() == 0)
			m_image_pI(width*height); // intialize the mandelbrot point colors of image, parallel
		time_length_p = timer(true); // start timer
		for (int y = 0; y < height; y++)
		{
			...
			{
				// add task to threadpool
				t_pool.post([=]() {m_image_p(x, y, x, y, task_id); });
				task_id++;
			}
		}
		t_pool.start(); // start the threadpool
...
// run thread test
void t_test(int t_number, bool isOnePixel)
{
	...
	while (sample < samples)
	{
		...
		task_run(t_number, isOnePixel); // create the mandelbrot image using threads in threadpool
		if (sample != samples - 1)
		{
...
int main()
{
	...
	{
		cout << "task test " << task << " of " << task_types << endl;
		if (i == 0) // 1 pixel
		{
			cout << "1 pixel" << endl;
		}
		else if (i == 1) // multiple pixels but less than 1 row
		{
			cout << "multiple pixels but less than 1 row" << endl;
			s_image_line(false);
		}
		else if (i == 2) // 1 row
		{
		...
		else if (i == 3) // multiple rows but less than evenly divided chunks
		{
			cout << "multiple rows but less than evenly divided chunks" << endl;
			s_image_block(4, 4, false);
		}
		else if (i == 4) // rows/n size chunks
		{
			cout << "rows/n size chunks" << endl;
			s_image_block(4, 4, true);
		}
		cout << endl;
